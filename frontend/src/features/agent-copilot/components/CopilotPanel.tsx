...
import type { ChangeProposal, EvidenceSource, GraphOperation, OperationPreview, HistoricalImprovementContext } from "../model/type"
...
export function CopilotPanel({ document, draft, draftVersion, history, initialSource, autoAnalyze = false, onApplyOperations, onPreviewCall, onProposalLoadingChange, onPreviewChange, onProposalErrorChange }: CopilotPanelProps) {
  ...
  const requestProposal = useCallback(async (source: EvidenceSource) => {
    ...
    clearPreview()
    try {
      const next = await createCopilotProposal({ document, baseVersion: draftVersion, source: { kind: source.kind, text: source.text.trim(), call: source.kind === "call" ? source.call : undefined }, history })
  ...
  }, [clearPreview, document, draft, draftVersion, history, onPreviewChange, onProposalErrorChange, onProposalLoadingChange])
