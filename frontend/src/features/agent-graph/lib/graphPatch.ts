...
import { defaultNodePosition } from "@/features/agent-graph/lib/graphLayout"
import type { GraphOperation, OperationPreview, OperationResult } from "../model/patch"
...
export function applyGraphOperation(draft: AgentDraft, operation: GraphOperation): { draft?: AgentDraft; errors: AgentValidationError[] } {
  ...
  switch (operation.op) {
    case "add_node": {
      ...
      next.layout[node.name] = operation.position ?? defaultNodePosition(next.config.nodes.length - 1)
      node.edges.forEach((edge, index) => {
        next.edgeHandles[edgeHandleKey(node.name, edge, index)] = defaultEdgeHandleLayout()
      })
      break
    }
    case "update_node": {
...
export function applyGraphOperations(draft: AgentDraft, operations: GraphOperation[], acceptedIndices = operations.map((_, index) => index)): OperationPreview {
  ...
  return { document: current.config, layout: current.layout, edgeHandles: current.edgeHandles, results, errors: [...operationErrors, ...documentErrors] }
}
