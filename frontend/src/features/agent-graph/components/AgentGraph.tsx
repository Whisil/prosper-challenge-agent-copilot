...
interface AgentGraphProps {
  ...
  isDirty: boolean
  readOnly?: boolean
  allowPreviewLayoutEdit?: boolean
  movableNodeNames?: string[]
}

function GraphContent({ config, layout, edgeHandles, selectedNodeName, onSelectNode, onCreateNode, onDeleteNode, onMoveNode, validationErrors, onCreateTransition, onSelectTransition, connectionInteraction, onStartConnection, onCancelConnection, isDirty, readOnly = false, allowPreviewLayoutEdit = false, movableNodeNames = [] }: AgentGraphProps) {
  ...
  const movableNames = useMemo(() => new Set(movableNodeNames), [movableNodeNames])
  const selectedNodes = useMemo(() => flowNodes.map((node) => ({ ...node, selected: node.id === selectedNodeName, draggable: !readOnly || movableNames.has(node.id) })), [flowNodes, movableNames, readOnly, selectedNodeName])
  ...
  return (
    <div className="relative h-full w-full canvas-grid">
      ...
      <ReactFlow
        ...
        onNodesChange={readOnly && !allowPreviewLayoutEdit ? undefined : onNodesChange}
        ...
        onNodeDragStop={readOnly && !allowPreviewLayoutEdit ? undefined : (_, node) => onMoveNode(node.id, node.position)}
        ...
        nodesDraggable={!readOnly || allowPreviewLayoutEdit}
