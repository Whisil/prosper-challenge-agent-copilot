...
export function NodeCreationDialog({ kind, nodes, onCancel, onSubmit }: NodeCreationDialogProps) {
  ...
  const typeDescription = kind === "conversation" ? "A focused spoken interaction where the agent asks questions, explains something, or gathers context." : kind === "tool" ? "A deterministic action such as searching availability or confirming a booking. This demo uses mocked tool metadata." : kind === "branch" ? "A routing decision that sends the conversation down a condition or fallback path." : kind === "transfer" ? "A controlled handoff to a human team with a reason and optional context." : kind === "end" ? "An explicit completion or failure point that ends the conversation." : "Set up the required details before adding this node to the graph."
