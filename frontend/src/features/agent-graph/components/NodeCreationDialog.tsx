...
export function NodeCreationDialog({ kind, nodes, onCancel, onSubmit }: NodeCreationDialogProps) {
  ...
  const instructionError = kind !== "transfer" && !instruction.trim() ? "Task instruction is required." : undefined
  ...
  const canSubmit = !nameError && !instructionError && !toolNameError && !toolDescriptionError && !transferError
  const typeLabel = kind === "end" ? "End" : kind ? kind.charAt(0).toUpperCase() + kind.slice(1) : "Node"
  const typeDescription = kind === "conversation" ? "A focused spoken interaction where the agent asks questions, explains something, or gathers context." : kind === "tool" ? "A mock action such as searching availability or confirming a booking. Configure its name and expected result for this demo." : kind === "transfer" ? "A controlled handoff to a human team with a reason and optional context. This node ends the automated call." : kind === "end" ? "An explicit completion or failure point that ends the conversation." : "Set up the required details before adding this node to the graph."
  ...
  const submit = () => {
    ...
    onSubmit({
      name: normalizedName,
      title: trimmedName,
      instruction: kind === "transfer" ? `Hand off the caller. Reason: ${transferReason.trim()}. Context: ${transferContext.trim()}` : instruction.trim(),
      roleMessage: kind === "transfer" ? undefined : roleMessage,
  ...
  return <Dialog open={kind !== null} onOpenChange={(open) => !open && onCancel()}>
    <DialogContent aria-describedby="node-creation-description">
      ...
      <div className="mt-5 space-y-4">
        ...
        {kind !== "transfer" && <><FormField label="Task instruction" info="What the voice agent should do while this node is active." error={submitted ? instructionError : undefined}><Textarea value={instruction} onChange={(event) => setInstruction(event.target.value)} placeholder="Describe what the agent should say, ask, or accomplish..." className="min-h-24" /></FormField><FormField label="Node-specific guidance" info="Optional guidance that replaces the global persona while this node is active."><Textarea value={roleMessage} onChange={(event) => setRoleMessage(event.target.value)} placeholder="Optional node-specific guidance..." className="min-h-16" /></FormField></>}
