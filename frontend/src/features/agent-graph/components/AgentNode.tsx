...
import { AlertCircle, ArrowRightLeft, CircleUserRound, Flag, MessageSquareText, PhoneOff, Wrench } from "lucide-react"
...
export function AgentNode({ data, selected, dragging }: NodeProps<FlowNode>) {
  ...
  const isTerminal = nodeType === "end" || nodeType === "transfer" || Boolean(node.end)
  const nodeTypeLabel = nodeType === "end" ? "End" : nodeType === "tool" ? "Tool" : nodeType === "transfer" ? "Handoff" : "Conversation"
  ...
  return (
    ...
    >
      {([ ["top", Position.Top], ["right", Position.Right], ["bottom", Position.Bottom], ["left", Position.Left] ] as const).map(([side, position]) => { const handleId = side === "left" ? "target" : `target-${side}`; const connected = data.connectedTargetHandleIds.includes(handleId); return <Handle key={`target-${side}`} id={handleId} className={cn("!z-10 !h-3.5 !w-3.5 !border-2 !border-white !bg-[#668c70] opacity-0 transition-[transform,box-shadow,opacity] group-hover:opacity-100", (connected || isConnectionTarget) && "!opacity-100", isConnectionTarget && "!h-4 !w-4 !bg-[#3f684b] !shadow-[0_0_0_5px_rgba(109,144,120,0.2)]", isInvalidConnectionTarget && "opacity-30")} position={position} type="target" /> })}
      ...
      <div className="flex items-center gap-2 border-b border-[#eef1ee] px-4 py-3">
        <div className={cn("flex h-7 w-7 items-center justify-center rounded-lg", isInitial ? "bg-[#e9f0ea] text-[#62816a]" : isTerminal ? "bg-[#f3ece8] text-[#a47663]" : "bg-[#f0f3ef] text-[#6d7f72]")}>
          {isInitial ? <Flag size={14} /> : nodeType === "end" ? <PhoneOff size={14} /> : nodeType === "tool" ? <Wrench size={14} /> : nodeType === "transfer" ? <ArrowRightLeft size={14} /> : <CircleUserRound size={15} />}
        </div>
      ...
      {!isTerminal && ([ ["top", Position.Top], ["right", Position.Right], ["bottom", Position.Bottom], ["left", Position.Left] ] as const).map(([side, position]) => { const handleId = side === "bottom" ? NEW_TRANSITION_HANDLE : `connection-${side}`; const connected = data.connectedSourceHandleIds.includes(handleId); return <Handle key={`source-${side}`} id={handleId} aria-label="Connect this node" role="button" title="Drag this dot to connect this node to another node" className={cn("!z-20 !h-3.5 !w-3.5 !border-2 !border-white !bg-[#668c70] opacity-0 transition-[transform,box-shadow,opacity] group-hover:opacity-100", connected && "!opacity-100", isConnectionSource && "!opacity-100 !h-4 !w-4 !bg-[#3f684b] !shadow-[0_0_0_5px_rgba(109,144,120,0.2)]")} position={position} type="source" /> })}
    </div>
  )
}
