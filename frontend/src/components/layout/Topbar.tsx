import { ChevronDown, Download, PhoneCall, Plus, Redo2, Save, Settings2, Undo2, Upload, CheckCircle2, Play } from "lucide-react"
...
export function Topbar({ agentName, persona, onUpdatePersona, onTestCall, isDirty, validationErrors, onSelectValidationError, onSave, onUndo, onRedo, canUndo, canRedo, onExport, onImport, onSimulate, onReset }: TopbarProps) {
  ...
  return (
    <header className="flex h-[72px] shrink-0 items-center justify-between border-b border-[#e5e8e4] bg-white px-7">
        ...
        <div>
          <div className="relative flex items-center gap-2">
            ...
            {agentMenuOpen && <div className="absolute left-0 top-full z-50 mt-2 w-64 rounded-xl border border-[#dfe4df] bg-white p-2 shadow-[0_16px_35px_rgba(31,48,40,0.16)]" role="listbox" aria-label="Agents">
              ...
              <Button type="button" size="sm" variant="outline" className="w-full justify-center" disabled title="Agent creation is coming soon"><Plus size={13} /> Add agent</Button>
      ...
      <div className="flex items-center gap-1.5">
        ...
        <Button size="sm" variant="ghost" onClick={onSimulate} title="Simulate draft"><Play size={14} /> Simulate</Button>
